...
def get_scaffolds(path_to_binary: str) -> list:
        ...
        """
        print("--- STEP 1 --- \n Extracting scaffold accession IDs from the input file...")
...
def get_assemblies(scaffolds: list) -> dict:
        ...
        for scaffold in scaffolds:
                ...
                if process.stdout().strip() in result.values():  # This if-statement checks if the genome assembly ID is a duplicate
                    continue
                else:
...
def dereplicate_genomes(assemblies: list) -> None:
        ...
        A file called "dereplicated_assemblies.txt" is generated.
...
def extract_cleaned_hits(dereplicated_assemblies: str, scaff_ass_pairs:dict) -> None:
...
def main():
        ...
        extract_cleaned_hits('dereplicated_assemblies.txt', scaff_ass_pairs)
